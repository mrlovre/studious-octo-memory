...
class Layer(metaclass=ABCMeta):
    ...
    # See documentation of abstract base classes (ABC):
...
class Convolution(Layer):
    ...
    def __init__(self, input_layer, num_filters, kernel_size, name, padding='SAME',
        ...
        if padding == 'SAME':
            ...
            # without padding
            self.shape = (
                N, num_filters, H - kernel_size + 1, W - kernel_size + 1)
            self.pad = 0
    ...
    def backward_inputs(self, grad_out):
        # nice trick from CS231n, backward pass can be done with just matrix
        # mul and col2im
        grad_out = grad_out.transpose(1, 2, 3, 0).reshape(self.num_filters, -1)
        grad_x_cols = self.weights.T.dot(grad_out)
        N, C, H, W = self.input_shape
        k = self.kernel_size
        grad_x = col2im_cython(
            grad_x_cols, N, C, H, W, k, k, self.pad, self.stride)
        return grad_x
...
class MaxPooling(Layer):
    ...
    def forward(self, x):
        ...
        # with this clever reshaping we can implement pooling where pool_size
        # == stride
...
class FC(Layer):
    ...
    def forward(self, inputs):
        ...
        """
        self.inputs = inputs
        return np.dot(inputs, self.weights.transpose()) + self.bias
    ...
    def backward_params(self, grads):
        ...
        """
        grad_weights = np.dot(self.inputs.transpose(), grads).transpose()
...
class L2Regularizer():
    ...
    def __init__(self, weights, weight_decay, name):
        ...
        Args:
          ...
          name: layer name
        """
        # this is still a reference to original tensor so don't change
        # self.weights
    ...
    def forward(self):
        ...
        """
        return self.weight_decay / 2 * np.linalg.norm(self.weights)
...
def softmax(x):
    ...
    exps = np.exp(x).transpose()
    return (exps / np.sum(exps, axis=0)).transpose()
